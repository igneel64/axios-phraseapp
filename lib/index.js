...
exports.attach = (axiosInstance, config = defaultConfig) => [
  (axiosInstance || axios).interceptors.request.use(onPhraseAppRequest(config)),
  (axiosInstance || axios).interceptors.response.use(
    onPhraseAppResponse(config)
  )
];
...
) => async (/** @type {import("axios").AxiosRequestConfig} */ axiosConfig) => {
